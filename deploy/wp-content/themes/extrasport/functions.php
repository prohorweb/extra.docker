...
/**
 * Inline guard that silences Yandex Maps "Invalid API key" console noise.
 *
 * @return void
 */
function extrasport_enqueue_yandex_maps_console_guard() {
	wp_register_script( 'yandex-maps-console-guard', false, array(), null, true );
	wp_enqueue_script( 'yandex-maps-console-guard' );
	wp_add_inline_script(
		'yandex-maps-console-guard',
		"(function(){var patterns=['Invalid API key','Yandex Maps JS API'];function shouldIgnore(value){var message=String(value||'');return patterns.some(function(pattern){return message.indexOf(pattern)!==-1;});}['warn','error'].forEach(function(level){var original=console[level];console[level]=function(){if(shouldIgnore(arguments[0])){return;}original.apply(console,arguments);};});})();"
	);
}
...
function extrasport_enqueue_scripts() {
    ...
    $main_deps = array();
    if ( extrasport_should_enqueue_yandex_maps() ) {
        extrasport_enqueue_yandex_maps_console_guard();

        wp_enqueue_script(
            'yandex-maps',
            extrasport_get_yandex_maps_script_url(),
            array( 'yandex-maps-console-guard' ),
            null,
            true
        );
...
require_once EXTRASPORT_DIR . '/inc/cards.php';
require_once EXTRASPORT_DIR . '/inc/admin-card-meta.php';
require_once EXTRASPORT_DIR . '/inc/admin-branding.php';
require_once EXTRASPORT_DIR . '/inc/classic-editor.php';
